...
public class ExtremeBasinRenderer extends SmartBlockEntityRenderer<ExtremeBasinBlockEntity> {
    public ExtremeBasinRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected void renderSafe(ExtremeBasinBlockEntity basin, float partialTicks, PoseStack ms, MultiBufferSource buffer,
        ...
        boolean outToBasin = basin.getLevel()
                .getBlockState(basin.getBlockPos()
                        .relative(direction))
                .getBlock() instanceof ExtremeBasinBlock;

        for (IntAttached<ItemStack> intAttached : basin.visualizedOutputItems) {
            float progress = 1 - (intAttached.getFirst() - partialTicks) / ExtremeBasinBlockEntity.OUTPUT_ANIMATION_TIME;
    ...
    protected float renderFluids(ExtremeBasinBlockEntity basin, float partialTicks, PoseStack ms, MultiBufferSource buffer,
                                 int light, int overlay) {
