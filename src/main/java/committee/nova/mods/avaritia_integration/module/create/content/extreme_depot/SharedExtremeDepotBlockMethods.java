...
public class SharedExtremeDepotBlockMethods {
    ...
    public static InteractionResult onUse(BlockState state, Level world, BlockPos pos, Player player, InteractionHand hand, BlockHitResult ray) {
        ...
        if (!mainItemStack.isEmpty() && !heldItem.isEmpty() && ItemStack.isSameItemSameTags(mainItemStack, heldItem)) {
            TransportedItemStack incoming = new TransportedItemStack(heldItem);
        ...
        if (!heldItem.isEmpty() && !shouldntPlaceItem) {
            TransportedItemStack transported = new TransportedItemStack(heldItem.copy());
