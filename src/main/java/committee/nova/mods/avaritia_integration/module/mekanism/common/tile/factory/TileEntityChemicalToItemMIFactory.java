...
public abstract class TileEntityChemicalToItemMIFactory<RECIPE extends MekanismRecipe<?>> extends TileEntityMIFactory<RECIPE> {
    ...
    protected void addGasTanks(ChemicalTankHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        ...
        for (int i = 0; i < tier.processes; i++) {
            int index = i;
            inputTank[i] = BasicChemicalTank.createModern(MAX_CHEMICAL * tier.processes, (type, automationType) -> automationType != AutomationType.EXTERNAL ||
                            (type.isRadioactive() && RadiationManager.isGlobalRadiationEnabled()), (stack, type) -> isValidInputChemical(stack),
                    stack -> isChemicalValidForTank(stack) && inputProducesOutput(index, stack, outputSlot[index], false), ChemicalAttributeValidator.ALWAYS_ALLOW,
    ...
    protected void sortInventoryOrTank() {
        Map<ChemicalStack, CIRecipeProcessInfo<RECIPE>> processes = new Object2ObjectOpenCustomHashMap<>(new Hash.Strategy<>() {
            @Override
            public int hashCode(ChemicalStack stack) {
                return stack.hashCode();
            }

            @Override
            public boolean equals(ChemicalStack first, ChemicalStack second) {
    ...
    protected static class CIRecipeProcessInfo<RECIPE extends MekanismRecipe<?>> {
        ...
        @Nullable
        private ToIntBiFunction<CIRecipeProcessInfo<RECIPE>, TileEntityChemicalToItemMIFactory<RECIPE>> lazyMinPerTank;
        private Object item;
        private RECIPE recipe;
        private long minPerTank = 1;
        private long totalCount;

        public long getMinPerTank(TileEntityChemicalToItemMIFactory<RECIPE> factory) {
